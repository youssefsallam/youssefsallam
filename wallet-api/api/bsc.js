module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (let page = 0; page < 10; page++) {
      ...
      if (!r.ok) {
        return res.status(r.status).json({ ok: false, stage: 'binance-web3', status: r.status, body: text.slice(0, 300) });
      }

      let json;
      try { json = JSON.parse(text); } catch {
        return res.status(502).json({ ok: false, stage: 'json', body: text.slice(0, 300) });
      }

      if (String(json.code) !== '000000') {
        return res.status(502).json({ ok: false, stage: 'api', code: json.code, message: json.message || null });
      }
      ...
      for (const token of list) {
        const price = Number(token && token.price || 0);
        const qty = Number(token && token.remainQty || 0);
        const usd = price * qty;
        if (Number.isFinite(usd) && usd >= 0.01) total += usd;
      }
